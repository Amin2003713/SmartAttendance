        ...
        IDailyCalendarQueryRepository
{
    public async Task<bool> IsAlreadyHoliday(Guid projectId, DateTime dateTime, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var isAlreadyHoliday = await TableNoTracking.AnyAsync(
                c => c.Date == dateTime &&
                     c.IsHoliday &&
                     c.CalendarProjects.Any(a => a.ProjectId == projectId),
                cancellationToken);

            var holiday = await calendarQueryRepository.IsAlreadyHoliday(projectId, dateTime, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var privateHolidays = await TableNoTracking.Where(e => e.Date >= startAt &&
                ...
                .Select(e => new GetHolidayResponse
                {
                    ...
                    Inserter = DbContext.Set<User>()
                        ...
                        .Select(u => new
                        {
                            u.Id, Name = u.FirstName + " " + u.LastName
                        })
                        .FirstOrDefault()!
                })
            ...
            var publicHolidays =
                await calendarQueryRepository.GetHolidaysForMonth(projectId, startAt, endAt, cancellationToken);
                ...
                .Select(e =>
                    new GetReminderResponse
                    {
                        ...
                        TargetUsers = e.CalendarUsers.Select(cu => new UserTargetRequest
                            {
                                Id = cu.UserId, Name = DbContext.Set<User>().First(u => u.Id == cu.UserId).FullName()
                            })
                            .ToList()
                    })
                .ToListAsync(cancellationToken);
