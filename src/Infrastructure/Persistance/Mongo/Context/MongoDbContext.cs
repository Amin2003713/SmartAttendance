...
public sealed class MongoDbContext
{
    ...
    private void EnsureIndexes()
    {
        ...
        {
            new CreateIndexModel<UserDocument>(Builders<UserDocument>.IndexKeys.Ascending(x => x.Email),
                new CreateIndexOptions
                {
                    Unique = true,
                    Name = "ux_users_email"
                }),
            new CreateIndexModel<UserDocument>(Builders<UserDocument>.IndexKeys.Ascending(x => x.NationalCode),
                new CreateIndexOptions
                {
                    Unique = true,
                    Name = "ux_users_ncode"
        ...
        {
            new CreateIndexModel<AttendanceDocument>(Builders<AttendanceDocument>.IndexKeys.Ascending(x => x.StudentId).Ascending(x => x.PlanId),
                new CreateIndexOptions
                {
                    Name = "ix_att_student_plan"
                }),
            new CreateIndexModel<AttendanceDocument>(Builders<AttendanceDocument>.IndexKeys.Ascending(x => x.RecordedAtUtc),
                new CreateIndexOptions
                {
                    Name = "ix_att_recorded"
                })
        });
