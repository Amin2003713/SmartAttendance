...
{
	public static UserDocument ToDocument(this UserAggregate user) => new()
	{
		Id = user.Id.Value,
	...
	{
		var agg = new UserAggregate(new UserId(doc.Id), doc.FirstName, doc.LastName,
			new EmailAddress(doc.Email), new PhoneNumber(doc.Phone), new NationalCode(doc.NationalCode));
		foreach (var rid in doc.RoleIds) agg.AssignRole(new RoleId(rid));
		return agg;
	}

	public static PlanDocument ToDocument(this PlanAggregate plan) => new()
	{
		Id = plan.Id.Value,
		Title = plan.Title,
	...
	public static PlanAggregate ToDomain(this PlanDocument doc)
	{
		...
		foreach (var s in doc.Students) agg.RegisterStudent(new UserId(s));
		return agg;
	}

	public static AttendanceDocument ToDocument(this AttendanceAggregate att) => new()
	{
		Id = att.Id.Value,
	...
	public static AttendanceAggregate ToDomain(this AttendanceDocument doc)
		=> new(new AttendanceId(doc.Id), new UserId(doc.StudentId), new PlanId(doc.PlanId));

	public static DocumentDocument ToDocument(this Document d) => new()
	{
		Id = d.Id.Value,
		FileName = d.FileName,
	...
	public static Document ToDomain(this DocumentDocument doc)
		=> new(new DocumentId(doc.Id), doc.FileName, doc.ContentType, doc.SizeBytes, doc.UploadedAtUtc, doc.AttendanceId.HasValue ? new AttendanceId(doc.AttendanceId.Value) : null);

	public static NotificationDocument ToDocument(this Notification n) => new()
	{
		Id = n.Id.Value,
