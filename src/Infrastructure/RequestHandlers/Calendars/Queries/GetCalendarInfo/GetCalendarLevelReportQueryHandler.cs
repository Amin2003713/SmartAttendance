...
using Shifty.Common.Messaging.Contracts.PRIMA.Items.Queries.PrimaCalendarLevelReport;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fetchTasks
            = await broker.RequestAsync<GetPrimaCalendarLevelReportBrokerResponse, GetPrimaCalendarLevelReportBroker>(
                ...
                {
                    Access = access.Adapt<GetProjectUserAccessBrokerResponse>(), ProjectId = request.ProjectId, Date = request.Date
                },
                cancellationToken);
    ...
    private static OrderedDictionary<UserType, string> ComputeBadge(List<IDictionary<UserType, ReviewAction>> allReviewStatuses, UserType node)
    {
        ...
        var userLevelRatios = allReviewStatuses
            ...
            .Select(group =>
            {
                ...
                return new
                {
                    node = group.Key, Ratio = $"{verified}/{total}"
                };
            })
            .ToList();
