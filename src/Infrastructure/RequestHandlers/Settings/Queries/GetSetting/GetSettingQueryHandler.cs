...
using Shifty.Application.Settings.Queries.GetSetting;
using Shifty.Common.Exceptions;

namespace Shifty.RequestHandlers.Settings.Queries.GetSetting;
    ...
    public async Task<GetSettingQueryResponse> Handle(GetSettingQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (result == null)
            {
                ...
                throw IpaException.NotFound(localizer["No Setting configuration found."]);
            }
        ...
        catch (Exception ex)
        {
            ...
            throw IpaException.InternalServerError(localizer["An unexpected error occurred while retrieving Setting."]);
        }
    }
}