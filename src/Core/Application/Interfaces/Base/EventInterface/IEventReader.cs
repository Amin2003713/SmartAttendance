    ...
    where TAggregate : AggregateRoot<TId>, new()
{
    ...
    Task<SnapShotDocument<TId>> LoadSnapShotAsync(TId aggregateId, CancellationToken cancellationToken = default);
    ...
    Task<bool> SnapShotExistsAsync(TId aggregateId, CancellationToken cancellationToken = default);
    ...
    /// <summary>
    ///     Similar to AnyAsync but kept for compatibility in sync-like scenarios.
    /// </summary>
    /// <example>
    ///     <code>
    /// bool result = await reader.Any(x => x.GeneralWorkerCount > 5);
    /// </code>
    /// </example>
    Task<bool> Any(
        Expression<Func<BaseEventStoreModel<TId>, bool>> predicate,
