...
public sealed record AssignRoleCommand(AssignRoleRequest Request) : IRequest;

// Handler: انتساب نقش
public sealed class AssignRoleCommandHandler(IUserRepository userRepository, IRoleReadService roleReadService, IUnitOfWork unitOfWork)
	: IRequestHandler<AssignRoleCommand>
{
	public async Task Handle(AssignRoleCommand request, CancellationToken cancellationToken)
	{
		var r = request.Request;
		var user = await userRepository.GetByIdAsync(new UserId(r.UserId), cancellationToken)
