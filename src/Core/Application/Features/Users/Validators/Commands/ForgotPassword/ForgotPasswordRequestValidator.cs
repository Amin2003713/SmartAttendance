...
public class ForgotPasswordRequestValidator : AbstractValidator<ForgotPasswordRequest>
{
    public ForgotPasswordRequestValidator(IStringLocalizer<ForgotPasswordRequestValidator> localizer)
    {
        ...
        RuleFor(x => x.NewPassword)
            ...
            .Matches(@"[A-Z]")
            .WithMessage(localizer["Password must contain at least one uppercase letter."].Value) // "رمز عبور باید حداقل یک حرف بزرگ داشته باشد."
            .Matches(@"[a-z]")
            .WithMessage(localizer["Password must contain at least one lowercase letter."].Value) // "رمز عبور باید حداقل یک حرف کوچک داشته باشد."
            .Matches(@"\d")
            ...
            .WithMessage(localizer[
                "Password must contain at least one special character."].Value); // "رمز عبور باید حداقل یک کاراکتر خاص داشته باشد."
        ...
        RuleFor(x => x.ConfirmPassword)
            ...
            .WithMessage(localizer[
                "Confirm password must match the new password."].Value); // "تایید رمز عبور باید با رمز عبور جدید مطابقت داشته باشد."
