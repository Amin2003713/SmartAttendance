...
public sealed class UpdateUserValidator : AbstractValidator<UpdateUserRequest>
{
	public UpdateUserValidator()
	{
		RuleFor(x => x.FirstName)
			...
			.MaximumLength(100).WithMessage("نام نباید بیش از ۱۰۰ کاراکتر باشد.");
		...
		RuleFor(x => x.LastName)
			...
			.MaximumLength(100).WithMessage("نام خانوادگی نباید بیش از ۱۰۰ کاراکتر باشد.");
		...
		RuleFor(x => x.Phone)
			...
			.Matches("^\\+?[0-9]{7,15}$").WithMessage("فرمت شماره تلفن نامعتبر است.");
	}
}
